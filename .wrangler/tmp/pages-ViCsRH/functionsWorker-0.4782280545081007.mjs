...
// api/[[route]].js
function scheduleBroadcast(context2, type, payload) {
  context2.waitUntil(broadcastEvent(context2.env, type, payload));
}
  ...
  try {
    ...
    if (path === "/barbers") {
      ...
      if (request.method === "POST") {
        ...
        scheduleBroadcast(context2, "BARBERS_UPDATED", formatted);
        return new Response(JSON.stringify(formatted), { headers });
      }
    ...
    if (path === "/services") {
      ...
      if (request.method === "POST") {
        ...
        const { results } = await env2.DB.prepare("SELECT * FROM services").all();
        scheduleBroadcast(context2, "SERVICES_UPDATED", results);
        return new Response(JSON.stringify(results), { headers });
      }
      if (request.method === "DELETE") {
    ...
    if (path === "/bookings") {
      ...
      if (request.method === "POST") {
        ...
        scheduleBroadcast(context2, "NEW_BOOKING", created);
      ...
      if (request.method === "PUT") {
        ...
        if (updatedBooking) {
          if (body.status && !body.date) {
            scheduleBroadcast(context2, "BOOKING_STATUS_CHANGED", { id: body.id, status: body.status, booking: updatedBooking });
          } else {
            scheduleBroadcast(context2, "BOOKING_RESCHEDULED", updatedBooking);
          }
        }
        return new Response(JSON.stringify(formatted), { headers });
      }
    ...
    if (path === "/notifications") {
      ...
      if (request.method === "POST") {
        ...
        if (newest) {
          scheduleBroadcast(context2, "NOTIFICATION_ADDED", newest);
        }
        return new Response(JSON.stringify(formatted), { headers });
      }
    }
...
// ../.wrangler/tmp/bundle-clG8ow/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-clG8ow/middleware-loader.entry.ts
