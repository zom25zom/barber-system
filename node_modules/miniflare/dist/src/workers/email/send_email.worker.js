...
var SendEmailBinding = class extends WorkerEntrypoint {
  ...
  getServiceBinding(bindingName) {
    let binding = this.env[bindingName];
    if (!binding)
   ...
   */
  async storeTempFile(content, extension, prefix, location = "system", messageUUID) {
    let body;
    ...
    let fileName = messageUUID ? `${messageUUID}.${extension}` : `${crypto.randomUUID()}.${extension}`, url = new URL(`${prefix}/${fileName}`, "http://placeholder/"), diskConfig = this.env.email_disk_services.find(
      (config) => config.location === location
    );
    if (!diskConfig)
      throw new Error(`Disk service for ${location} not found`);
    return await this.getServiceBinding(diskConfig.bindingName).fetch(url, {
      method: "PUT",
      body
    }), joinPath(diskConfig.path, prefix, fileName);
  }
  ...
  async send(emailMessageOrBuilder) {
    ...
    if (this.isEmailMessage(emailMessageOrBuilder)) {
      ...
      let locations = this.env.email_disk_services.map(
        (service) => service.location
      ), filePaths = await Promise.all(
        locations.map(
          (location) => this.storeTempFile(
            rawEmailBuffer,
            "eml",
            "email",
            location,
      ...
      ), projectIndex = locations.indexOf("project"), fileInfo = `Email: ${filePaths[projectIndex !== -1 ? projectIndex : 0]}`;
      return this.log(
...
${fileInfo}`
      ...
      if (builder.text) {
        let text = builder.text, textResults = await Promise.all(
          locations.map(
            (location) => this.storeTempFile(text, "txt", "email-text", location, messageUUID)
          )
        ), projectIndex = locations.indexOf("project"), logIndex = projectIndex !== -1 ? projectIndex : 0;
        files.push(`Text: ${textResults[logIndex]}`);
      }
      if (builder.html) {
        let html = builder.html, htmlResults = await Promise.all(
          locations.map(
            (location) => this.storeTempFile(
              html,
              "html",
      ...
      if (builder.attachments)
        for (let attachment of builder.attachments) {
          let extMatch = attachment.filename.match(/\.([^.]+)$/), extension = extMatch ? extMatch[1] : "bin", attachmentUUID = crypto.randomUUID(), attachmentResults = await Promise.all(
            locations.map(
              (location) => this.storeTempFile(
                attachment.content,
                extension,
